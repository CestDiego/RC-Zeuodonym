...
/***/ function(module, exports, __webpack_require__) {
	...
	var pseudoRegexp = RegExp('\\b(' + _Object$keys(_data2['default']).map(function (key) {
	  return _data2['default'][key];
	}).join('|') + ')\\b', 'g');

	function getRealName(name) {
	  return _lodash2['default'].filter(_Object$keys(_data2['default']), function (key) {
	    return _data2['default'][key] === name;
	  })[0];
	}
	...
	function hookStuff() {
	  var _loop = function (i) {
	    ...
	    var showPseudonym = function showPseudonym() {
	      var leName = senderNameElements[i].innerHTML;

	      senderNameElements[i].innerHTML = leName.replace(nameRegexp, function (_, word) {
	        return _data2['default'][word];
	      });
	      senderNameElements[i].parentNode.parentNode.removeEventListener('mouseover', showPseudonym);
	    };
	    var showRealName = function showRealName() {
	      var leName = senderNameElements[i].innerHTML;

	      senderNameElements[i].innerHTML = leName.replace(pseudoRegexp, function (_, word) {
	        return getRealName(word);
	      });
	      senderNameElements[i].parentNode.parentNode.removeEventListener('mouseout', showRealName);
	    };
