...
/***/ function(module, exports, __webpack_require__) {
	...
	var active = true;
	var nameRegexp = RegExp('\\b(' + _Object$keys(_data2['default']).join('|') + ')\\b', 'g');
	var pseudoRegexp = RegExp('\\b(' + _lodash2['default'].map(_Object$keys(_data2['default']), function (name) {
	  return _data2['default'][name];
	}).join('|') + ')\\b', 'g');
	var bigArrayOfAllNames = [];
	var getRealName = function getRealName(pseudo) {
	  return pseudo.replace(pseudoRegexp, function (match, pseudonym) {
	    return _lodash2['default'].filter(_Object$keys(_data2['default']), function (name) {
	      return _data2['default'][name] === pseudonym;
	    })[0];
	  });
	};
	var getPseudo = function getPseudo(fullName) {
	  var pseudo = fullName.replace(nameRegexp, function (match, fullName) {
	    return _data2['default'][fullName];
	  });
	...
	var getListObserver = function getListObserver() {
	    ...
	    mutations.map(function (mutation) {
	      ...
	      for (var i = 0; i < nodes.length; ++i) {
	        ...
	        if (id && id.indexOf('message_group') !== -1) {
	          var nameElements = item.getElementsByClassName('sender_name');
	          var messageLabel = item.querySelector('.message_label_clickable');
	...
	var chatListObserver = getListObserver();
	var privateListObserver = getListObserver();

	var sidebarObserver = new MutationObserver(function (mutations) {
	  return _lodash2['default'].map(mutations, function (mutation) {
	    var nodes = mutation.addedNodes;
	    ...
	    for (var i = 0; i < nodes.length; ++i) {
	      ...
	      if (className && className.indexOf('user_sidebar_entry') !== -1) {
	        var userName = item.querySelector('a');
	...
	var restorePseudo = function restorePseudo() {
	  return _lodash2['default'].map(bigArrayOfAllNames, function (elem) {
	    return elem.innerHTML = getPseudo(elem.innerHTML);
	  });
	};
	var restoreNamesToNormal = function restoreNamesToNormal() {
	  return _lodash2['default'].map(bigArrayOfAllNames, function (elem) {
	    return elem.innerHTML = getRealName(elem.innerHTML);
	  });
	};
	var turnOnPseudonyms = function turnOnPseudonyms() {
	  var listConfig = {
	    childList: true,
	    subtree: false,
	    attributes: false,
	  ...
	  if (bigArrayOfAllNames > 0) restorePseudo();
	...
	var turnOffPseudonyms = function turnOffPseudonyms() {
	  disconnectObservers();
	  restoreNamesToNormal();
	};

	var togglePsudonyms = function togglePsudonyms() {
	  return active ? turnOffPseudonyms() : turnOnPseudonyms();
	};

	chrome.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
	  if (msg.action === "whatsup") sendResponse({ action: "status", content: active });else if (msg.action === "toggle") {
	    togglePsudonyms();
	    active = !active;
